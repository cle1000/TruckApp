...
if (init != 1){
	...
	startPage.innerHTML = '<div data-role="header" data-position="fixed">	<h1>mto</h1></div><!-- /header --><div role="main" class="ui-content">	<form action="#home" method="post" id="first_infos">		<ul data-role="listview" data-inset="true">			<fieldset>				<legend class="first">Ihre Daten...</legend>				<label for="name_input">Name</label> <input name="name"					id="name_input" type="text" placeholder="Name" /> <label					for="telephone_input">Telefon</label> <input name="telephone"					id="telephone_input" type="text" placeholder="Telefon">			</fieldset>		</ul>		<div class="ui-grid-a">			<div class="ui-block-a">				<button class="ui-shadow ui-btn ui-corner-all" type="submit"					onClick="saveUserInfos()" id="submit_input">Speichern</button>			</div>			<div class="ui-block-b">				<button class="ui-shadow ui-btn ui-corner-all" type="submit"					onClick="setFirstStart()" id="submit_input">Überspringen</button>			</div>		</div>	</form></div>';
